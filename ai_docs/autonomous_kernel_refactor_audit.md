...
| R4 | Central CapabilityBroker and PolicyEngine for fs/process/git/AWS/cloud/package/network/subagent/future effects | Partial | `src/policy/engine.ts`, `src/capabilities/broker.ts`, `src/capabilities/manifest.ts`, `src/capabilities/filesystem/provider.ts`, `src/shell.ts`, `src/aws-cli.ts`, `src/git.ts`, `src/subagents.ts`; browser/MCP provider effects still need full broker enforcement |
...
| R7 | Workflow kernel extraction and `index.ts` composition root target | Partial | `src/kernel/workflow-engine.ts`, `src/kernel/output-synthesis.ts`, `src/workspace/change-set.ts`, `src/review/gates/release-gate.ts`, `src/ui/commands.ts`, `src/ui/goal-commands.ts`, `src/ui/tools.ts`, `src/ui/tool-interception.ts`; `src/index.ts` is reduced but lifecycle orchestration still remains |
