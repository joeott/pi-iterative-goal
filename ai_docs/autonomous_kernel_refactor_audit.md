...
| R4 | Central CapabilityBroker and PolicyEngine for fs/process/git/AWS/cloud/package/network/subagent/future effects | Partial | `src/policy/engine.ts`, `src/capabilities/broker.ts`, `src/capabilities/manifest.ts`, `src/shell.ts`, `src/aws-cli.ts`, `src/git.ts`; direct filesystem/provider families still need full broker routing |
