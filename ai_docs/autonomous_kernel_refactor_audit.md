...
| R3 | Typed domain objects and runtime validation at model/tool/plugin boundaries | Partial | `src/domain/*`, `src/evaluator.ts`, `src/capabilities/manifest.ts`, `src/capabilities/broker.ts`; broader plugin/provider contract tests pending |
| R4 | Central CapabilityBroker and PolicyEngine for fs/process/git/AWS/cloud/package/network/subagent/future effects | Partial | `src/policy/engine.ts`, `src/capabilities/broker.ts`, `src/capabilities/manifest.ts`, `src/shell.ts`, `src/git.ts`; AWS/direct providers not fully broker-routed |
...
| R7 | Workflow kernel extraction and `index.ts` composition root target | Partial | `src/kernel/workflow-engine.ts`, `src/workspace/change-set.ts`, `src/review/gates/release-gate.ts`, `src/ui/commands.ts`, `src/ui/tools.ts`; `src/index.ts` is reduced but lifecycle orchestration still remains |
